...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def find(sentence):
        tokens = split(lower(remove_punctuation(sentence)))
        for word in topic:
            if word in tokens:
                return True
        return False
...
def shifty_shifts(start, goal, limit):
    ...
    if start[0] != goal[0] and limit == 0:
        return 1
    return (start[0] != goal[0]) + shifty_shifts(start[1:], goal[1:], limit - (start[0] != goal[0]))
    # END PROBLEM 6
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[times_per_player[i][index] - times_per_player[i][index-1]
                for index in range(1, len(times_per_player[i]))]
...
def fastest_words(game):
    ...
    for word_index in words:
        word_time = [time(game, player_num, word_index) for player_num in players]
        min_pleyer_num = word_time.index(min(word_time))
        res[min_pleyer_num].append(word_at(game, word_index))
    return res
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    if start == goal:
        ...
        if start[0] == goal[0]:
            ...
            substitute_diff = key_distance_diff(start[1:], goal[1:], limit-sub_diff) + sub_diff
