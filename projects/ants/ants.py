...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant is None and (not isinstance(self, ContainerAnt)):
                place.ant.contained_ant = self
            elif (not isinstance(place.ant, ContainerAnt)) and isinstance(self, ContainerAnt) and self.contained_ant is None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        ptr = self.place
        while distance < self.min_range and ptr is not beehive:
            ptr = ptr.entrance
            distance += 1
        while distance <= self.max_range and ptr is not beehive:
            if ptr.bees:
...
# END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.armor <= amount and self.true_queen:
            bees_win()
        else:
            Ant.reduce_armor(self, amount)
        # END Problem 13
